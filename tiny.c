...
{
    int listenfd, *connfd, port, clientlen,i,main_thread_priority,main_policy;
    ...
    main_thread_priority = sched_get_priority_max(SCHED_FIFO); //get max priority of RR scheduling
    main_thread_param.sched_priority= ((int)(main_thread_priority/2) + 2 ); 	// set main priority to one more than half of max priority
	...
	for(i=0;i<3;i++)
		thread_cr8(i);   // main thread returns
...
void *thread_consumer(void *arg){
	...
	pthread_getschedparam (w_priority, &policy, &param);
	 ...
	 connfd = get();   /* connected socket to service */
         pthread_cond_signal(&empty); // signal manager of consumption	 
       	 pthread_mutex_unlock(&mutex); // 
         //thread_ptr[(int) arg].thread_count++; //increase thread count
...
void thread_cr8(int i) {
	...
	my_param.sched_priority = ((int)(sched_get_priority_max(SCHED_FIFO)/2 ) - 2) ; // get max priorty&set priorty to one less than half
	pthread_attr_setschedparam(&worker_attr, &my_param); // set priority  
